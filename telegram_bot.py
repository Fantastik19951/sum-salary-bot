...
async def safe_edit(msg:Message, text:str, kb:InlineKeyboardMarkup):
    try:
        ...
        logging.warning(f"safe_edit→edit failed: {e}")
...
async def show_profit(msg,ctx,s,e,title,push=True):
    if push: push_nav(ctx,title,title)
    ents=[e for v in ctx.application.bot_data["entries"].values() for e in v if s<=pdate(e['date'])<=e and "amount" in e]
    tot=sum(e["amount"] for e in ents)
    text=f"{title} ({sdate(s)} – {sdate(e)})\n<b>10%: {fmt_amount(tot*0.10)} $</b>"
    await safe_edit(msg,text,nav_kb(ctx))

async def show_kpi(msg,ctx,prev=False,push=True):
    if prev:
        s,e=bounds_prev(); title="📊 KPI прошлого"
    else:
        s,e=bounds_today(); title="📊 KPI текущего"
    if push: push_nav(ctx,title,title)
    ents=[e for v in ctx.application.bot_data["entries"].values() for e in v if s<=pdate(e['date'])<=e and "amount" in e]
    if not ents:
...
async def ask_date(msg,ctx):
    p=await msg.reply_text("📅 Введите дату или «Сегодня»",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Сегодня",callback_data="today_add")]]))
    ctx.user_data["flow"]={"step":"date","msg":msg,"prompt":p}

async def ask_name(msg,ctx):
    p=await msg.reply_text("✏️ Введите имя:")
    ctx.user_data["flow"].update({"step":"sym","prompt":p})

async def ask_amount(msg,ctx,prev=None):
    txt="💰 Введите сумму:" if prev is None else f"💰 Новая сумма (старое: {fmt_amount(prev)} $):"
    p=await msg.reply_text(txt)
    ctx.user_data["flow"].update({"step":"val","prompt":p})
...
async def process_text(u:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if flow["step"]=="val":
        ...
        date=flow["date"]; code=f"{pdate(date).year}-{pdate(date).month:02d}"
        ...
        if flow.get("mode")=="edit":
            ...
            ctx.application.bot_data["entries"]=read_sheet()
            r=await flow["msg"].reply_text("✅ Изменено",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↺ Отменить",callback_data=f"undo_edit_{idx}")]]))
            ctx.user_data["undo_edit"]={"row":idx,"old_symbols":flow["old_symbols"],"old_amount":flow["old_amount"],"expires":dt.datetime.utcnow()+dt.timedelta(seconds=UNDO_WINDOW)}
        ...
        row=push_row(flow)
        ctx.application.bot_data["entries"]=read_sheet()
        r=await flow["msg"].reply_text(f"✅ Добавлено: {flow['symbols']} · {fmt_amount(val)} $",
...
async def cb(upd:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("edit_"):
        _,r,code,day=data.split("_",3); idx=int(r)
        old=next(e for e in ctx.application.bot_data["entries"].get(code,[]) if e["row_idx"]==idx)
        ctx.user_data["flow"]={"step":"sym","mode":"edit","row":idx,"date":day,"old_symbols":old["symbols"],"msg":msg}
    ...
    if data.startswith("undo_edit_"):
        i=int(data.split("_",1)[1]); ud=ctx.user_data.get("undo_edit",{})
        if ud.get("row")==i and dt.datetime.utcnow()<=ud.get("expires"):
            update_row(i,ud["old_symbols"],ud["old_amount"])
            ctx.application.bot_data["entries"]=read_sheet()
            return await show_main(msg,ctx)
        return await msg.reply_text("⏱ Время вышло")

    if data.startswith("undo_"):
        i=int(data.split("_",1)[1]); ud=ctx.user_data.get("undo",{})
        if ud.get("row")==i and dt.datetime.utcnow()<=ud.get("expires"):
            delete_row(i); ctx.application.bot_data["entries"]=read_sheet()
            return await show_main(msg,ctx)
        return await msg.reply_text("⏱ Время вышло")
    ...
    if data=="back":
        c,l=pop_view(ctx)
        if c=="main": return await show_main(msg,ctx,push=False)
        if c.startswith("year_"): return await show_year(msg,ctx,c.split("_",1)[1],push=False)
        if c.startswith("mon_"): return await show_month(msg,ctx,c.split("_",1)[1],None,push=False)
        if c.startswith("day_"):
            _,cc,dd=c.split("_",2)
            return await show_day(msg,ctx,cc,dd,push=False)
        if c=="hist": return await show_history(msg,ctx,push=False)
        return await show_main(msg,ctx,push=False)

    if data.startswith("year_"):   return await show_year(msg,ctx,data.split("_",1)[1])
    if data.startswith("mon_"):    return await show_month(msg,ctx,data.split("_",1)[1])
    if data.startswith("tgl_"):    _,cc,fl=data.split("_",2); return await show_month(msg,ctx,cc,fl)
    if data.startswith("day_"):    _,cc,dd=data.split("_",2); return await show_day(msg,ctx,cc,dd)
    if data=="go_today":
        ctx.application.bot_data["entries"]=read_sheet()
    ...
    if data.startswith("drow_"):
        _,r,cc,dd=data.split("_",4)[:4]; delete_row(int(r))
        ctx.application.bot_data["entries"]=read_sheet()
...
async def cmd_start(u:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    # Добавляем в чаты
    ctx.application.bot_data["chats"].add(u.effective_chat.id)
