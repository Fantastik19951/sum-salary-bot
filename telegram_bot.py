...
# Новая show_month с ReplyKeyboard
async def show_month(msg:Message,ctx,code,flag=None,push=True):
    year,mon=code.split("-")
    ...
    kb=[[KeyboardButton("Первая половина" if flag=="old" else "Вторая половина")]]
    for d in days:
        kb.append([KeyboardButton(d)])
    kb.append([KeyboardButton("⬅️ Назад"), KeyboardButton("🏠 Главное")])

    try:
        await msg.edit_text(header+body+footer,parse_mode="HTML",
...
# Новая show_day с ReplyKeyboard
async def show_day(msg:Message,ctx,code,date,push=True):
    ...
    for i,e in enumerate(ents, start=1):
        kb.append([KeyboardButton(f"❌ {i}")])
        kb.append([KeyboardButton(f"✏️ {i}")])
    kb.append([KeyboardButton("➕ Добавить запись")])
    kb.append([KeyboardButton("⬅️ Назад"), KeyboardButton("🏠 Главное")])
...
async def show_history(msg,ctx,push=True):
    ...
    if not ents:
        ...
        text="<b>📜 История ЗП</b>\n"+"\n".join(lines)
    await safe_edit(msg,text,InlineKeyboardMarkup(nav_kb(ctx).inline_keyboard))
...
async def show_kpi(msg,ctx,prev=False,push=True):
    ...
    ents=[e for v in ctx.application.bot_data["entries"].values() for e in v 
          if start<=pdate(e['date'])<=end and "amount" in e]
    if not ents:
        return await safe_edit(msg,"Нет данных",InlineKeyboardMarkup(nav_kb(ctx).inline_keyboard))
...
async def cb(upd:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    # (Inline edit/add logic остаётся без изменений — копируйте из Ideal 1)
    # ...
    # для сокращения здесь не повторяем, но оставляем
    # show_main, show_year, show_history, show_profit, show_kpi
    # а также undo и edit inline через q.data
    ...
    # и т.д. — скопируйте ветки из вашего Ideal 1

# ─── REPLY-HANDLER ДЛЯ МЕНЮ МЕСЯЦ/ДЕНЬ ───────────────────────────────────────
async def cb_reply(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    text=update.message.text
    # month-menu:
    if re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", text):
        # пользователь нажал на дату
        code=ctx.user_data["nav"][-1][0].split("_",1)[1]
        return await show_day(update.message,ctx,code,text)
    if text in ("Первая половина","Вторая половина"):
        # toggle половины
        code=ctx.user_data["nav"][-1][0].split("_",1)[1]
        return await show_month(update.message,ctx,code, "old" if text=="Первая половина" else "new")
    if text=="➕ Добавить запись":
        # добавить запись в текущий день
        period,day=ctx.user_data["nav"][-1][0].split("_",2)[1:]
        return await ask_date(update.message,ctx)
    if text=="⬅️ Назад":
        # emulate inline «back»
        code,label=pop_view(ctx)
        if code=="main":         return await show_main(update.message,ctx,push=False)
        if code.startswith("year_"): return await show_year(update.message,ctx,code.split("_",1)[1],push=False)
        if code.startswith("mon_"):  return await show_month(update.message,ctx,code.split("_",1)[1],None,push=False)
        if code.startswith("day_"):
            _,c,dd=code.split("_",2)
            return await show_day(update.message,ctx,c,dd,push=False)
        return await show_main(update.message,ctx,push=False)
    if text=="🏠 Главное":
        return await show_main(update.message,ctx)
    # удаление/редактирование через Reply:
    if m:=re.fullmatch(r"❌ (\d+)",text):
        i=int(m.group(1))-1
        code,day=ctx.user_data["nav"][-1][0].split("_",2)[1:]
        ents=ctx.application.bot_data["entries"][code]
        row=ents[i]["row_idx"]
        delete_row(row)
        ctx.application.bot_data["entries"]=read_sheet()
        return await show_day(update.message,ctx,code,day)
    if m:=re.fullmatch(r"✏️ (\d+)",text):
        i=int(m.group(1))-1
        code,day=ctx.user_data["nav"][-1][0].split("_",2)[1:]
        ents=ctx.application.bot_data["entries"][code]
        old=ents[i]
        ctx.user_data["flow"]={
            "mode":"edit","row":old["row_idx"],
            "old_symbols":old["symbols"],"old_amount":old["amount"],
            "date":day,"period":code,"msg":update.message
        }
        return await ask_name(update.message,ctx)
    # иначе попадаем в текстовый flow
...
if __name__=="__main__":
    ...
    # обработчик для Reply-кнопок месяца/дня
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, cb_reply))
    # и для текстовых ответов flow
