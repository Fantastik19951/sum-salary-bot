...
async def show_day(msg,ctx,code,date,push=True):
    ...
    for i,e in enumerate(ents):
        rows.append([
            ...
            InlineKeyboardButton(f"✏️{i+1}",
                                 callback_data=f"edit_{e['row_idx']}_{code}_{date}")
        ])
...
async def show_kpi(msg,ctx,prev=False,push=True):
    ...
    # прогноз: 10% от суммы заполненных дней * (15 / days_filled)
...
async def process_text(u:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if flow["step"]=="val":
        ...
        if flow.get("mode")=="edit":
            ...
            return await show_day(flow["msg"],ctx,period,date)
