...
REMIND_HH_MM = (20, 0)  # 20:00 daily reminder
MONTH_NAMES  = [
...
def main_kb():
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"{PAD*5}💰 Текущая ЗП", callback_data="profit_now"),
         InlineKeyboardButton(f"{PAD*5}💼 Прошлая ЗП", callback_data="profit_prev")],
        [InlineKeyboardButton("📜 История ЗП", callback_data="hist")],
        [InlineKeyboardButton("📊 KPI текущего", callback_data="kpi"),
         InlineKeyboardButton("📊 KPI прошлого", callback_data="kpi_prev")],
    ])
...
async def show_profit(msg, ctx, start, end, title, push=True):
    ...
    text = f"<b>{title}</b> ({sdate(start)} – {sdate(end)})\n<b>10%: {fmt_amount(tot*0.10)} $</b>"
    await safe_edit(msg, text, nav_kb(ctx))
...
async def show_kpi(msg, ctx, prev=False, push=True):
    ...
    sal  = turn * 0.10
    # считаем только дни с записями
    filled_days = len({e['date'] for e in ents})
    avg = sal / filled_days if filled_days else 0
    period_len = 15
    # прогноз на 15 дней на основе avg
    fc = round(avg * period_len, 2)
    # для прошлого KPI ограничиваем прогноз зарплатой
    if prev and fc > sal: fc = sal
    text = (
        f"<b>{title}</b> ({sdate(start)} – {sdate(end)})\n"
        f"• Оборот: {fmt_amount(turn)} $\n"
        f"• ЗП10%: {fmt_amount(sal)} $\n"
        f"• Дней заполнено: {filled_days}/{period_len}\n"
...
async def process_text(u:Update, ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if flow["step"]=="val":
        try:
            ...
            return await flow["msg"].reply_text("❌ Нужна числовая сумма")
        date = flow["date"]
        code = date[:7]  # YYYY-MM
...
async def cb(upd:Update, ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if d == "back":
        ...
        # из KPI/History сразу в Главное
        if code in ("kpi","kpi_prev","hist"):
            return await show_main(msg, ctx, push=False)
        if code=="main":
            return await show_main(msg, ctx, push=False)
    ...
    if d.startswith("edit_"):
        ...
        ctx.user_data["flow"] = {
            "mode":"edit","row":idx,"date":day,"period":code,
