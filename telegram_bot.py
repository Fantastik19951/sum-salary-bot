...
async def show_year(msg: Message, ctx, year: str):
...
async def show_month(msg: Message, ctx, code: str, flag=None):
    year,mon = code.split("-")
...
async def show_day(msg: Message, ctx, code: str, date: str):
    ctx.application.bot_data["entries"] = read_sheet()
...
async def show_history(msg: Message, ctx):
...
async def show_profit(msg: Message, ctx, start, end, title: str):
...
async def show_kpi(msg: Message, ctx, prev: bool):
    if prev:
...
async def cb(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("undo_edit_"):
        _,row = data.split("_",1)
        row = int(row)
        uflow = ctx.user_data.get("flow", {})
        # can't undo after new flow started, skip
        update_row(row, uflow.get("old_symbols",""), uflow.get("old_amount",0))
        ctx.application.bot_data["entries"] = read_sheet()
