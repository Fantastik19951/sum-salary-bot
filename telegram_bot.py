...
async def show_month(msg, ctx, code, flag=None):
    ...
    body="\n".join(f"{e['date']} · {e['symbols']} · ${e['amount']:.2f'}" for e in part) or "Нет записей"
    await safe_edit(
        msg,
...
async def show_day(msg, ctx, code, date):
    ...
    body="\n".join(f"{i}. {e['symbols']} · ${e['amount']:.2f'}"
...
async def show_history(msg, ctx):
    ...
    for e in sorted(ents, key=lambda x:pdate(x["date"])):
        d=pdate(e["date"])
        sal=e["salary"]
        lines.append(f"• {d.day} {MONTH_NAMES[d.month-1]} {d.year} — ${sal:.2f}")
    body="\n".join(lines)
...
async def cb(upd:Update, ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("edit_"):
        ...
        ctx.user_data["flow"]={
            "step":"sym","mode":"edit","row":row,"date":d
        }
