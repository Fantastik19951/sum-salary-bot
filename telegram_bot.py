...
HEADER_ROWS  = 4
UNDO_WINDOW  = 10    # seconds before undo message disappears
REMIND_HH_MM  = (20, 0)
MONTH_NAMES  = [
...
def push_nav(ctx, code):
    ctx.user_data.setdefault("nav", deque()).append(code)

def peek_nav(ctx):
    nav = ctx.user_data.get("nav", deque())
    return nav[-2] if len(nav) >= 2 else "main"
...
async def show_day(msg, ctx, code, date):
    push_nav(ctx, f"day_{code}_{date}")
    ctx.application.bot_data["entries"] = read_sheet()
...
async def cb(upd:Update, ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("undo_"):
        ...
        if ud.get("row")==row and dt.datetime.utcnow() <= ud.get("expires", dt.datetime.min):
    ...
    if data.startswith("undo_edit_"):
        ...
        if ud.get("row")==row and dt.datetime.utcnow() <= ud.get("expires", dt.datetime.min):
            update_row(row, ud["old_symbols"], ud["old_amount"])
            ctx.application.bot_data["entries"]=read_sheet()
            return await show_main(msg, ctx)
        else:
            return await msg.reply_text("⏱ Время вышло")
