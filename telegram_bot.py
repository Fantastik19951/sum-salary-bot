...
def pop_nav(ctx):
    ...
    if len(nav) > 1:
        nav.pop()
    return nav[-1] if nav else ("main","Главное")

def nav_kb(ctx):
    prev_code, prev_label = pop_nav(ctx)
    return InlineKeyboardMarkup([[
        InlineKeyboardButton(f"⬅️ {prev_label}", callback_data=prev_code),
...
async def safe_edit(msg: Message, text: str, kb=None):
    kb = kb or nav_kb(msg.bot_data if False else {})  # placeholder
    try:
...
def fmt_amount(x: float) -> str:
    ...
    # else show 2 decimals, strip trailing zero
...
async def show_history(msg, ctx):
    ...
    if not ents:
        ...
        text = "<b>📜 История ЗП</b>\n" + "\n".join(lines)
    await msg.edit_text(text, parse_mode="HTML", reply_markup=nav_kb(ctx))
...
async def show_kpi(msg, ctx, prev=False):
    ...
    if not ents:
        return await msg.edit_text("Нет данных", parse_mode="HTML", reply_markup=nav_kb(ctx))
...
async def cb(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # BACK button is just the callback_data of previous view
    if data == "main":
