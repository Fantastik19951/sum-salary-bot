...
HEADER_ROWS  = 4
UNDO_WINDOW  = 10    # сек. до удаления "Запись добавлена" и "Запись удалена"
REMIND_HH_MM = (20, 0)
...
async def show_month(m, ctx, code, flag=None):
    ...
    all_e = ctx.bot_data["entries"].get(code, [])
    tx = [e for e in all_e if "amount" in e]
...
async def process_text(u: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    step, txt = ad["step"], u.message.text.strip()
    ...
    if step=="val":
        ...
        except: return await u.message.reply_text("Нужно ввести число")
...
async def cb(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if code.startswith("drow_"):
        _,row,mc,dd=code.split("_"); delete_row(int(row)); ctx.bot_data["entries"]=read_sheet()
        return await show_day(m, ctx, mc, dd)
