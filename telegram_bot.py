...
async def show_month(msg, ctx, code, flag=None):
    ...
    cb, label = f"mon_{code}", f"{mname} {year}"
    ...
    body   = "\n".join(
        f"{e['date']} · {e['symbols']} · {fmt_amount(e['amount'])} $"
        for e in part
    ) or "Нет записей"
...
async def show_day(msg, ctx, code, date):
    cb, label = f"day_{code}_{date}", date
    push_nav(ctx, cb, label)
    ...
    header = f"<b>{date}</b>"
    body   = "\n".join(
        f"{i}. {e['symbols']} · {fmt_amount(e['amount'])} $"
...
async def show_history(msg, ctx):
    ...
    if not ents:
        ...
        for e in sorted(ents, key=lambda x: pdate(x["date"])):
            d = pdate(e["date"])
            lines.append(f"• {d.day} {MONTH_NAMES[d.month-1]} {d.year} — {fmt_amount(e['salary'])} $")
        text = "<b>📜 История ЗП</b>\n" + "\n".join(lines)
...
async def ask_rec(msg, ctx):
    flow = {"step": "date", "prompt": None, "origin_msg": msg}
...
async def ask_fixed(msg, ctx, code, date):
    flow = {"step": "sym", "mode": "add", "date": date, "prompt": None, "origin_msg": msg}
    prompt = await msg.reply_text("✏️ Введите имя:")
    flow["prompt"] = prompt
    ctx.user_data["flow"] = flow

async def ask_sal(msg, ctx):
    flow = {"step": "val", "mode": "salary", "date": sdate(dt.date.today()), "prompt": None, "origin_msg": msg}
...
async def process_text(u:Update, ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if flow["step"] == "val":
        ...
        origin_msg, code, date = flow["origin_msg"], flow["date"][:7], flow["date"]
        ...
        if flow.get("mode") == "edit":
            ...
            return await show_day(origin_msg, ctx, code, date)
...
async def cb(upd:Update, ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("tgl_"):
        ...
        return await show_month(msg, ctx, code, flag=fl)
    ...
    if data.startswith("undo_"):
        ...
        origin_msg, code, day = ctx.user_data.get("last_day", (msg,) + ("", ""))
        if udata.get("row") == row and dt.datetime.utcnow() <= udata.get("expires"):
            delete_row(row)
            ctx.application.bot_data["entries"] = read_sheet()
            try:    await msg.delete()
            except: pass
            ...
            return await show_day(origin_msg, ctx, code, day)
        else:
