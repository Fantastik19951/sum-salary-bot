...
async def show_month(msg,ctx,code,flag=None,push=True):
    ...
    cb=f"mon_{code}"; label=f"{mname} {year}"
    if push: push_nav(ctx,cb,label)
    today=dt.date.today()
    if flag is None:
...
async def show_day(msg,ctx,code,date):
    cb=f"day_{code}_{date}"; label=date
    push_nav(ctx,cb,label)
    # store for update after add/edit
...
async def show_kpi(msg,ctx,prev=False):
    if prev:
        start,end=bounds_prev(); title="📊 KPI прошлого периода"; code_lab="kpi_prev"
    else:
        start,end=bounds_today();   title="📊 KPI текущего периода"; code_lab="kpi"
    push_nav(ctx,code_lab,title)
    ents=[e for v in ctx.application.bot_data["entries"].values() for e in v if start<=pdate(e['date'])<=end and "amount" in e]
    if not ents:
...
async def process_text(u:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if flow["step"]=="val":
        ...
        if flow.get("mode")=="edit":
            ...
            ctx.user_data["undo_edit"]={"row":idx,
                "old_symbols":flow["old_symbols"],
                "old_amount":flow["old_amount"],
                "expires":dt.datetime.utcnow()+dt.timedelta(seconds=UNDO_WINDOW)
            }
            # refresh day
            last_msg,code,day=ctx.user_data.get("last_day", (resp,flow["date"][:7],flow["date"]))
            return await show_day(last_msg,ctx,code,day)
        # ADD
        flow["amount"]=val
        row=push_row(flow)
        ctx.application.bot_data["entries"]=read_sheet()
...
async def cb(upd:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    # record last_day msg only on day view
    if data.startswith("day_") or data=="go_today":
        ctx.user_data["last_day"]=(msg,)
    ...
    # PROFIT / HISTORY / KPI
    if data=="profit_now":
        s,e=bounds_today(); return await show_profit(msg,ctx,s,e,"💰 Текущая ЗП","profit_now")
    if data=="profit_prev":
        s,e=bounds_prev(); return await show_profit(msg,ctx,s,e,"💼 Прошлая ЗП","profit_prev")
    if data=="hist":
