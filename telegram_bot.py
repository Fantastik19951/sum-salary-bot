    ...
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(
                f"{PAD*5}💰 Текущая ЗП{PAD*10}",
                callback_data="profit_now"
            ),
            InlineKeyboardButton(
                f"{PAD*5}💼 Прошлая ЗП{PAD*10}",
                callback_data="profit_prev"
            )
        ],
...
async def show_history(msg, ctx, push=True):
    ...
    if not ents:
        ...
        lines = [
            f"• {pdate(e['date']).day} {MONTH_NAMES[pdate(e['date']).month-1]} "
