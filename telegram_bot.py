...
def main_kb():
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(f"{pad*4}📆 Сегодня{pad*4}", callback_data="go_today")],
        [InlineKeyboardButton(f"{pad*4}➕ Запись{pad*4}", callback_data="add_rec")],
        [InlineKeyboardButton(f"{pad*4}💵 Зарплата{pad*4}", callback_data="add_sal")],
        [
        ...
        [InlineKeyboardButton(f"{pad*4}📜 История ЗП{pad*4}", callback_data="hist")],
        [
            InlineKeyboardButton(f"{pad}📊 KPI тек.{pad}", callback_data="kpi"),
            InlineKeyboardButton(f"{pad}📊 KPI прошл.{pad}", callback_data="kpi_prev"),
        ],
    ])
...
async def ask_rec(msg, ctx):
    ...
    prompt = await msg.reply_text("📅 Введите дату (ДД.MM.ГГГГ):", reply_markup=kb)
...
async def process_text(u: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # DATE step
    if flow["step"] == "date":
        if txt.lower() == "сегодня" or txt.lower() == "today":
