...
def main_kb():
    PAD = "\u00A0"*2
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{PAD*3}📅 2024{PAD*3}", "year_2024"),
         InlineKeyboardButton(f"{PAD*3}📅 2025{PAD*3}", "year_2025")],
        [InlineKeyboardButton(f"{PAD*5}📆 Сегодня{PAD*5}", "go_today")],
        [InlineKeyboardButton(f"{PAD*5}➕ Запись{PAD*5}", "add_rec")],
        [InlineKeyboardButton(f"{PAD*5}💵 Зарплата{PAD*5}", "add_sal")],
        [InlineKeyboardButton(f"{PAD*3}💰 Текущая ЗП{PAD*5}", "profit_now"),
         InlineKeyboardButton(f"{PAD*3}💼 Прошлая ЗП{PAD*5}", "profit_prev")],
        [InlineKeyboardButton(f"{PAD*5}📜 История ЗП{PAD*5}", "hist")],
        [InlineKeyboardButton(f"{PAD*3}📊 KPI тек.{PAD*5}", "kpi"),
         InlineKeyboardButton(f"{PAD*3}📊 KPI прош.{PAD*5}", "kpi_prev")],
    ])
...
async def show_kpi(msg, ctx, prev=False, push=True):
    ...
    # строго прогноз = avg_per_day * days_filled в период, т.е. не учитываем пустые дни
    forecast = round(avg_per_day * days_filled, 2) if prev else round(avg_per_day * ((end-start).days+1),2)
    text = (
...
async def cb(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("edit_"):
        ...
        ctx.user_data["flow"] = {
            "mode":"edit","row":idx,
            "old_symbols":old["symbols"],"old_amount":old.get("amount",old.get("salary",0)),
            "date":day,"msg":msg
        }
    ...
    if d.startswith("undo_edit_"):
        ...
        if ud.get("row")==idx and dt.datetime.utcnow() <= ud.get("expires"):
            ...
            ctx.application.bot_data["entries"] = read_sheet()
            period = ud["date"][:7]
            return await show_day(msg, ctx, period, ud["date"])
    ...
    if d.startswith("day_"):
        ...
        return await show_day(msg, ctx, code, dd)
    if d == "go_today":
        ctx.application.bot_data["entries"] = read_sheet()
        today = dt.date.today()
        ds = sdate(today)
    ...
    if d == "add_sal":
        # salary flow—можно реализовать аналогично, если нужно
        return await ask_amount(msg, ctx)
    if d == "profit_now":
        start,end = bounds_today()
        return await show_profit(msg, ctx, start, end, "💰 Текущая ЗП")
    if d == "profit_prev":
        start,end = bounds_prev()
        return await show_profit(msg, ctx, start, end, "💼 Прошлая ЗП")
    if d == "hist":
...
if __name__ == "__main__":
    ...
    logger.info("🚀 Bot up — polling mode")
